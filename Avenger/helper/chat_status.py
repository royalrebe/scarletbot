...
from Avenger import (BOT_ID, GROUP_ANONYMOUS_BOT, DRAGONS, pbot,
                    TELEGRAM_SERVICES_IDs)
from Avenger.modules.mongo.connection_mongo import GetConnectedChat
...
async def isUserBanned(chat_id: int, user_id: int) -> bool:
    ...
    """
    data_list = await pbot.get_chat_members(
        chat_id=chat_id,
        filter='kicked'
        )
